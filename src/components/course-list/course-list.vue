  ...
  methods: {  
    getCourseList(token,pull,callback){      
      ...
      that.loadingData=true;     
       $.ajax({
        method:'get',
        url:BASE_URL2+`/partyCourse/getCourseListById?id=${this.$route.params.categoryId}&token=${token}&loginMark=${that.loginMark}&data=${JSON.stringify({pageSize:10,pageIndex:this.pageIndex})}`,
      ...
      // this.$http.get(BASE_URL2+`/partyCourse/getCourseListById?id=${this.$route.params.categoryId}&token=${token}&loginMark=api&data=${JSON.stringify({pageSize:10,pageIndex:this.pageIndex})}`)
      //   .then(res=>{
    ...
    that.$nextTick(() => {
      if (!that.scroll) {
        ...
        that.scroll.on('scroll', (pos) => { 
          console.log(pos)                   
          if(pos.y<(that.scroll.maxScrollY - 30)){
             that.pageIndex++;
