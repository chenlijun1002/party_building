...
export default {
  ...
  methods: {
    ...
    getCategoryList(token,callback){
      ...
      $.ajax({
        method:'get',
        url:BASE_URL2+`/dataitem/details?data=PartyAntiCorruptionCategory&token=${token}&loginMark=${that.loginMark}`,
        ...
        // this.$http.get(BASE_URL2+`/dataitem/details?data=PartyAntiCorruptionCategory&token=${token}&loginMark=api`)
        // .then(res=>{
    ...
    getNoticeList(token,pull,callback){      
      ...
      that.loadingData=true;
       $.ajax({
        method:'get',
        url:BASE_URL2+`/partyanticorruption/list?data=${parmes}&token=${token}&loginMark=${that.loginMark}`,
        success:function(res){
          if(res.code==200){ 
    ...
    //     }
    //   })
    //   // this.$http.get(BASE_URL2+`/partyanticorruption/list?data=${parmes}&token=${token}&loginMark=api`)
    //   //   .then(res=>{            
    //   //     if(callback && typeof callback=='function') callback();
    //   //     this.loading2=false;             
    //   //     if(pull=='pull'){//上拉加载更多            
